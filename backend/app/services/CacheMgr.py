    ...
    async def set(
        ...
        if isCommit:
            ...
            async with self.lock:
                ...
                logger.debug(
                    f"Added operation to pending queue for wid {wid}: {nid}, {cache_key}"
                )
    ...
    async def batchcommit(self, wid: str = None):
        ...
        async with self.lock:
            ...
            if wid is not None:
                ...
                logger.debug(
                    f"Batch committing {total_ops} operations for all workflows"
                )

            # 复制要提交的操作，以便在锁外进行数据库操作
            operations_to_commit = {}
            for current_wid in wids_to_commit:
                operations_to_commit[current_wid] = self.pending_operations[
                    current_wid
                ].copy()
