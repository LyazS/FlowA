...
import type { VFProvider } from '@/schemas/plugin_schemas'
  ...
  const importAllNodes = async () => {
    ...
    for (const provider in response) {
      for (const plugin of response[provider].Plugins) {
        ...
        if (createInfo) {
          // 存储节点创建函数
          AllNodeCreateFuncs.value[createInfo.NType] = () => createVFNodeFromData(createInfo)
          // 创建测试节点实例
          const testNode = AllNodeCreateFuncs.value[createInfo.NType]()
          AllTestNodes.value[createInfo.NType] = testNode
          // 如果视图类型不存在，则添加对应的组件
    ...
    logger.debug('All nodes imported', AllNodeCreateFuncs.value, AllTestNodes.value, AllVFNodeTypes)
  }
